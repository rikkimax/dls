...
final class FirstPass : ASTVisitor
{
	...
	void processTypeIdentifierPart(SemanticSymbol* symbol, TypeLookup* lookup, VariableContext* ctx, VariableContext.TypeInstance* current, TypeIdentifierPart tip)
	{
		...
		{
            writeln("tiptip:", tip.identifierOrTemplateInstance);
        ...
        if (tip.indexer)
        {
            writeln("*missing* indexed: ", tip.indexer.tokens);
            current.calltip ~= "[";
            foreach(tk; tip.indexer.tokens)
            {
                current.calltip ~= tk.text;
            }
            current.calltip ~= "]";
        }
	}
		...
		if (targs.namedTemplateArgumentList)
		{
			foreach(i, targ; targs.namedTemplateArgumentList.items)
			{
				...
				foreach(suffix; targ.type.typeSuffixes)
				{
					...
					{
                        writeln("huidfhudishfiudsifudshfiudshfiudshfiusdhf");
                        current.calltip ~= "[]";
                    }
	...
	void buildChain(SemanticSymbol* symbol, TypeLookup* lookup, VariableContext* ctx, TypeIdentifierPart tip)
	{
        ...
        {
            writeln("here too??");
            lookup.breadcrumbs.insert(istring("[]"));
        }
	}
	...
	void traverseUnaryExpression(SemanticSymbol* symbol, TypeLookup* lookup, VariableContext* ctx, UnaryExpression ue)
	{
		if (PrimaryExpression pe = ue.primaryExpression)
		{
			...
			if (pe.basicType != tok!"")
			{
                writeln("add pe.basicType: ", pe.basicType);
    ...
    void process_it(SemanticSymbol* symbol, TypeLookup* lookup, TypeIdentifierPart typeIdentifierPart)
    {
        if (typeIdentifierPart.identifierOrTemplateInstance)
        {
            writeln("## building calltip");
            foreach(tk; typeIdentifierPart.identifierOrTemplateInstance.tokens)
            {
                writeln("  ct tk: ", tk);
            }
            lookup.ctx.calltip = buildCalltip(typeIdentifierPart.identifierOrTemplateInstance.tokens);
            // hack indexer static array
        }

        processTypeIdentifierPart(symbol, lookup, &lookup.ctx, lookup.ctx.root, typeIdentifierPart);

        writeln("final: ", lookup.ctx.calltip);
        writeln("final: ", lookup.ctx.root.calltip);
        ...
        //    {
        //        lookup.ctx.calltip ~= "[";
        //        writeln("array::: ", suffix.tokens);
        //        foreach(tk; suffix.tokens)
        //        {
        //            writeln("  tk: ", tk);
        //            lookup.ctx.calltip ~= tk.text;
        //        }
        //        lookup.ctx.calltip ~= "]";
        //    }
        //    else if (suffix.star != tok!"")
        //    {
        //        writeln("a starrrr???");
        //        lookup.ctx.calltip = lookup.ctx.root.calltip ~  "*";
        //    }
        //}
    }
	...
	override void visit(const VariableDeclaration dec)
	{
		...
		foreach (declarator; dec.declarators)
		{
			...
			{
				if (auto une = cast(UnaryExpression) declarator.initializer.nonVoidInitializer.assignExpression)
				{
			...
			if (dec.type && dec.type.type2 && dec.type.type2.typeIdentifierPart)
			{
                ...
                //lookup.breadcrumbs.insert(istring("fuck"));
                ...
                process_it(symbol, lookup, typeIdentifierPart);
			}
		}
		if (dec.autoDeclaration !is null)
		{
			foreach (part; dec.autoDeclaration.parts)
			{
                ...
                foreach(c; lookup.breadcrumbs[])
                    original ~= c;
                writeln("## var auto: ", symbol.acSymbol.name);
                writeln("## lookup breadcrumbs: ", lookup.breadcrumbs[]);
                scope(exit) writeln("-- auto end");
                ...
                writeln(lookup.ctx.calltip);
                if(lookup.ctx.root)
                {
                    writeln(lookup.ctx.root.calltip);
                    writeln(lookup.ctx.root.chain);
                }
                //exit(0);
...
private:
		...
		const Type type, TypeLookup* l = null)
	{
		...
		else if (t2.typeIdentifierPart !is null)
		{
            ...
            writeln("writeIotcTo for ", currentSymbol.acSymbol.name);
            writeIotcTo(t2.typeIdentifierPart, lookup.breadcrumbs);

            //if (t2.typeIdentifierPart.indexer)
            //{
            //    lookup.ctx.calltip = t2.typeIdentifierPart.indexer.tokens[0].text;

            //    writeln("tip indexer: ", lookup.ctx.calltip);
            //}
        }
		else
		{
		...
		foreach (suffix; type.typeSuffixes)
		{
			...
			else if (suffix.array)
			{
                if (suffix.tokens.length > 2)
                {
                    ...
                    lookup.ctx.calltip = suffix.tokens[1].text;
                    writeln(" > arr_sym >2 ", suffix.tokens.length);
                    foreach(tk; suffix.tokens)
                    {
                        writeln("  tk: ", tk);
                    }
                ...
                else
                {
                    ...
                    writeln(" > arr_sym ", suffix.tokens.length);
                    foreach(tk; suffix.tokens)
                    {
                        writeln("  tk: ", tk);
                    }
...
void writeIotcTo(T)(const TypeIdentifierPart tip, ref T output)
{
	if (!tip.identifierOrTemplateInstance)
		return;
