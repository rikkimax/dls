...
struct Scope
{
    ...
    bool parentHasVersion(string value)
    {
        auto it = &this;

        while (it)
        {
            if (it.version_ == value) return true;
            it = it.parent;
        }
        return false;
    }

    bool relevantToCurrentOs()
    {
        version (Windows)
        {
            if (parentHasVersion("linux")) return false;
            else if (parentHasVersion("FreeBSD")) return false;
            else if (parentHasVersion("OpenBSDBSD")) return false;
            else if (parentHasVersion("Darwin")) return false;
        }
        return true;
    }
