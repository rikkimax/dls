...
int runServer(string[] args)
{
	...
	serverLoop: while (true)
	{
		...
		else
		{
			...
			else if (request.kind & RequestKind.symbolDetails)
			{
				info("Getting symbol detail");
				s.trySendResponse(getDocFull(request, cache), "Could not get symbol detail");
			}
			...
			else if (request.kind & RequestKind.extractCalltips)
			{
				info("Getting function calltips");
				s.trySendResponse(getFunctionCalltips(request, cache), "Could not get function calltips");
			}
