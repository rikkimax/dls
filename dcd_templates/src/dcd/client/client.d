...
int runClient(string[] args)
{
	...
	try
	{
		getopt(args, "cursorPos|c", &cursorPos, "I", &addedImportPaths,
			...
			"symbolLocation|l", &symbolLocation, "doc|d", &doc, "symbolDetails", &symbolDetails,
            "inlayHints", &inlayHints, "extractCalltips", &extractCalltips,
	...
	if (symbolLocation | getIdentifier)
		...
		request.kind |= RequestKind.symbolDetails;
	else if (inlayHints)
		request.kind |= RequestKind.inlayHints;
	else if (extractCalltips)
		request.kind |= RequestKind.extractCalltips;
	else if (search)
	...
	if (symbolLocation)
		...
		printSymbolDetailsResponse(response, fullOutput);
	else if (inlayHints)
		printInlayHintsResponse(response);
	else if (extractCalltips)
		printExtractCalltipsResponse(response);
...
Options:
    ...
    --fullDoc
        Gets detailed information about a type, including its documentation if present, at the cursor
        location.
...
void printDocResponse(ref const AutocompleteResponse response, bool extended)
{
    if (extended)
    {
        writeln(response.completions.length);
...
void printSymbolDetailsResponse(ref const AutocompleteResponse response, bool extended)
{
    writeln(response.completions.length);
	foreach (ref completion; response.completions)
    {
...
void printExtractCalltipsResponse(ref const AutocompleteResponse response)
{
	auto app = appender!(string[])();
	foreach (ref completion; response.completions)
    {
